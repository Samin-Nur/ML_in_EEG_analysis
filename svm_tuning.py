...
# Algorithms
def Support_Vector_Machine(X_train, y_train, X_test, y_test, num_class,kernal_info,C_info,cls_wt):
    print(" ")
    print("----------------------------------------------")
    algo_name = 'Support Vector Machine'
    svm_model_linear = SVC(kernel=kernal_info, C=C_info,class_weight=cls_wt).fit(X_train, y_train)
    svm_predictions = svm_model_linear.predict(X_test)
    accuracy=PRAF(y_test, svm_predictions, num_class, algo_name)
    return accuracy
...
def main():
    ...
    accuracy = Support_Vector_Machine(trainX, trainY, testX, testY, num_class, 'linear', 0.6, {1:1,2:1,3:3,4:1,5:1})
